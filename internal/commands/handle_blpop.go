...
func handleBlpop(serverCtx *ServerContext, handlerCtx *HandlerContext) resp.Value {
	...
	timeoutInSeconds, ok := handlerCtx.Cmd.ArgFloat(1)
	if !ok {
	...
	el, ok, timeout := serverCtx.Store.Blpop(key, timeoutInSeconds)
